...
class _OrderPageState extends State<OrderPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final rows = await _api.adminOrdersRaw();
      if (!mounted) return;
  ...
  List<Map<String, dynamic>> get _filtered =>
      _orders.where((o) => (o['status'] ?? '').toString() == widget.status).toList();

  String _itemsLabel(Map<String, dynamic> o) {
    final raw = (o['items'] as List?) ?? const [];
    final names = raw
        .whereType<Map>()
        .map((it) => (it['name'] ?? '').toString())
        .where((s) => s.isNotEmpty)
        .toList();
    if (names.isEmpty) return '${raw.length} item(s)';
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      _loading
                          ? 'Loading…'
                          : (_error != null ? 'Could not load orders' : 'There are ${_filtered.length} orders'),
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return Card(
                                    child: ListTile(
                                      ...
                                      subtitle: Text(
                                        [
                                          if (buyer.isNotEmpty) 'Buyer: $buyer',
                                          _itemsLabel(o),
